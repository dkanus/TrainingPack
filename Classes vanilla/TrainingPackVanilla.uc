class TrainingPackVanilla extends Mutator
    config(TrainingPackVanilla);
...
simulated function Tick(float Delta){
    ...
    if(!bInteractionAdded){
        ...
        if(PC != none){
            TInt = TrainInteraction(PC.Player.InteractionMaster.AddInteraction("TrainingPackVanilla.TrainInteraction", PC.Player));
...
function bool CheckReplacement(Actor Other, out byte bSuperRelevant){
    ...
    if(Role == ROLE_Authority && monster != none){
        NewHitbox = Spawn(class'TrainingPackVanilla.TrainHeadHitbox', monster);
        if(NewHitbox != None)
...
function Mutate(string MutateString, PlayerController KFController){
    ...
    if(command ~= "PREFS"){
        ...
        class'TrainingPackVanilla.Helper'.static.TellAbout(mod, KFController, Self);
    }
...
function SetFakedPlayers(int newFakedPlayers, bool bNewForce){
    ...
    if(fakedPlayers != newFakedPlayers){
