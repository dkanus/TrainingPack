class TrainingPack extends Mutator
    config(TrainingPack);
...
function PostBeginPlay(){
    ...
    else if(TrainNiceGT(KFGT) != none)
        TrainNiceGT(KFGT).Mut = Self;
    else
...
simulated function Tick(float Delta){
    ...
    if(!bInteractionAdded){
        ...
        if(PC != none){
            TInt = TrainInteraction(PC.Player.InteractionMaster.AddInteraction("TrainingPack.TrainInteraction", PC.Player));
...
function bool CheckReplacement(Actor Other, out byte bSuperRelevant){
    ...
    isBoss = (ZombieBoss(Other) != none) || (NiceZombieBoss(Other) != none);
    // Add head hit-box
    if(Role == ROLE_Authority && monster != none){
        NewHitbox = Spawn(class'TrainingPack.TrainHeadHitbox', monster);
        if(NewHitbox != None)
    ...
    if(bChangeHealth && monster != none){
        ...
        if(niceMonster != none){
            niceMonster.InitFlameFuel = niceMonster.FuelRatio * niceMonster.HealthMax;
            niceMonster.FlameFuel = niceMonster.InitFlameFuel;
        }
...
function Mutate(string MutateString, PlayerController KFController){
    ...
    if(command ~= "PREFS"){
        ...
        if(bShowHitboxesClient || bShowHitboxesServer){
            ...
            else if(HBoxSize == HBSIZE_NORMAL || NiceGT == none)
                BroadcastToAll("  Normal online scaling applied to zeds' hit-boxes.");
            else
                BroadcastToAll("  Extended online scaling applied to zeds' hit-boxes.");
        }
        if(bNoDrama)
    ...
    else if(command ~= "HELP" || command ~= "HLP"){
        class'TrainingPack.Helper'.static.TellAbout(mod, KFController, Self);
    }
...
function FillupMagazines(PlayerController PC, optional bool bSuppressCheat){
    ...
    local NiceWeapon NiceWeaponToFill;
    PlayerPawn = KFHumanPawn(PC.Pawn);
    if((bLockCheats && !bSuppressCheat) || PlayerPawn == none)
        return;
    if(!bSuppressCheat)
    ...
    for(Inv = PlayerPawn.Inventory; Inv != none; Inv = Inv.Inventory){
        ...
        NiceWeaponToFill = NiceWeapon(Inv);
        if(WeaponToFill != none){
            WeaponToFill.MagAmmoRemaining = WeaponToFill.MagCapacity;
            if(NiceWeaponToFill != none){
                if(NiceWeaponToFill.bHasChargePhase)
                    NiceWeaponToFill.MagAmmoRemaining ++;
                NiceWeaponToFill.ClientSetMagSize(WeaponToFill.MagAmmoRemaining, true);
            }
        }
    }
}
...
function Rescue(KFHumanPawn PlayerPawn, Pawn Villain){
    ...
    // If this is NicePack - remove bleeding and poison
    if(NiceGT != none){
        // No poison
        ...
        // No bleeding
        MRI = class'MeanReplicationInfo'.static.findSZri(PlayerPawn.PlayerReplicationInfo);
        if(MRI != none)
            MRI.stopBleeding();
    }
    ...
    if(SafeGuardReaction == REACTION_KILL){
        foreach PlayerPawn.RadiusActors(class'KFMonster', M, SafeGuardKillRadius){
            notBoss = (ZombieBoss(M) == none) && (NiceZombieBoss(M) == none);
            if(notBoss)
                M.KilledBy(M);
        }
        if(bSafeGuardRestoreKilled)
...
function SetHitBoxSize(EHitboxSize newSize){
    if(newSize != HBoxSize){
        ...
        if(HBoxSize == HBSIZE_HARD)
            ...
            BroadcastToAll("Extended online scaling will be applied to zeds' hit-boxes.");
    }
}
