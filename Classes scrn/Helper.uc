...
static function TellAbout(String arg, PlayerController KFController, TrainingPackScrN TrainMut){
    if(arg == ""){
        ...
        TrainMut.SendMessage("%r[CHEAT]%b MINLEVEL%w/%bMAXLEVEL:%w sets min or max level for the current game.", KFController);
        ...
        TrainMut.SendMessage("%bFAKED %y# %b{VANILLA}", KFController);
        TrainMut.SendMessage("    This command sets amount of faked players. Not a cheat.", KFController);
        TrainMut.SendMessage("    %bVANILLA%w modifier only has effect with %pScrN Balance%w active and forces usage of regular method besides %pScrN Balance%w's one," @
            "which should lead to a faster spawn rate.", KFController);
    }
    ...
    else if(arg ~= "MAXLEVEL" || arg ~= "MINLEVEL"){
        TrainMut.SendMessage("%bMAXLEVEL%w/%bMINLEVEL %y#", KFController);
        TrainMut.SendMessage("    These commands sets min or max level for the current game. Only works with %pScrN Balance%w running.", KFController);
        TrainMut.SendMessage("    In case %bminimal level%w > %bmaximum level%w after using one of these commands -" @
            "both parameters will be set to the same value.", KFController);
        TrainMut.SendMessage("    Not considered a cheat as long as you keep maximum level %y< 7%w.", KFController);
    }
    else if(arg ~= "SLOMO"){
